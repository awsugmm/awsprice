...
//GetWorkSpace getprice
func GetWorkSpace() {
	workspace := &schema.AmazonWorkSpaces{}
	...
	data := []*schema.AmazonWorkSpaces_Product{}
	for _, price := range workspace.Products {
		data = append(data, price)
	}

	for _, p := range data {
		for _, term := range workspace.Terms {
			if term.Sku == p.Sku {
