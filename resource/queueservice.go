...
//GetQueueService getprice
func GetQueueService() {
	quesvc := &schema.AWSQueueService{}
	err := quesvc.Refresh()
	if err != nil {
		panic(err)
	}
	...
	data := []*schema.AWSQueueService_Product{}
	for _, price := range quesvc.Products {
		data = append(data, price)
	}

	for _, p := range data {
		for _, term := range quesvc.Terms {
			if term.Sku == p.Sku {
