...
//GetCodeBuild getprice
func GetCodeBuild() {
	codebuild := &schema.CodeBuild{}
	err := codebuild.Refresh()
	if err != nil {
		panic(err)
	}
	...
	data := []*schema.CodeBuild_Product{}
	for _, price := range codebuild.Products {
		data = append(data, price)
	}

	for _, p := range data {
		for _, term := range codebuild.Terms {
			if term.Sku == p.Sku {
