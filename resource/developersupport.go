...
//GetDevSupport getprice
func GetDevSupport() {
	devsupport := &schema.AWSDeveloperSupport{}
	err := devsupport.Refresh()
	if err != nil {
		panic(err)
	}
	...
	data := []*schema.AWSDeveloperSupport_Product{}
	for _, price := range devsupport.Products {
		data = append(data, price)
	}

	for _, p := range data {
		for _, term := range devsupport.Terms {
			if term.Sku == p.Sku {
